...
def write_build_npz(
    ...
    *,
    ...
    shape = np.asarray(blocks.shape, dtype=np.int32)
    original = np.asarray(original_shape or tuple(blocks.shape), dtype=np.int32)
    np.savez_compressed(
        Path(output_path),
        blocks=blocks,
        shape=shape,
...
def write_structure_nbt(
    ...
    *,
    ...
    crop_shape = original_shape or tuple(int(v) for v in blocks.shape)
    size_y, size_z, size_x = (int(v) for v in crop_shape)
    cropped = blocks[:size_y, :size_z, :size_x]
        ...
        for z in range(size_z):
            for x in range(size_x):
                ...
                if block_id not in palette_index:
                    ...
                    palette_entry = {"Name": StringTag(block_name)}
                    if properties:
